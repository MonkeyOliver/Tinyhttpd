...
#define SA struct sockaddr  //模仿《unp》

typedef unsigned short u_short;  // here is something diff from source code

void *accept_request(void *);
void cat(int, FILE *);         // ac
void error_die(const char *);  // ac
int get_line(int, char *, int);
void headers(int, const char *);  // ac
void not_found(int);              // ac
void serve_file(int, const char *);
int startup(u_short *);   // ac
void unimplemented(int);  // ac
...
void *accept_request(void *arg) {
     ...
     * stat——《APUE》4.2节
     */
    printf("parse success and path is %s\n", path);
    if (stat(path, &st) == -1) {
...
void cat(int client, FILE *resource) {
    ...
    // A question, why do not use do-while or just put this line in while
...
int main(int argc, char **argv) {
    ...
    while (1) {
        if ((client_sock = accept(server_sock, (SA *)&client_name,
                                  &client_name_len)) == -1)
            error_die("accept");
        else
